 *Need to install grc1leg package 
...
bysort DG_role: reg x2 x_y1 x21 if include_data==1, clu(subj_id)
tabstat own_f if include_data==1, stats(p25 p50 p75) save
gen stat_q=.
mat rr=r(StatTotal)
replace stat_q=1 if own_f<rr[1,1]
replace stat_q=2 if own_f>=rr[1,1]&own_f<rr[2,1]
replace stat_q=3 if own_f>=rr[2,1]&own_f<rr[3,1]
replace stat_q=4 if own_f>=rr[3,1]
tab stat_q if own_f!=.&include_data==1


bysort stat_q: reg x2 x_y1 x21 if include_data==1, clu(subj_id)
gen x_y1_statq=x_y1*stat_q
gen x21_statq=x21*stat_q
reg x2 x_y1 x_y1_statq x21 x21_statq stat_q if include_data==1&inlist(stat_q,2,4), clu(subj_id) pformat(%5.4f)
reg x2 x_y1 x_y1_statq x21 x21_statq stat_q if include_data==1&inlist(stat_q,1,4), clu(subj_id) pformat(%5.4f)
reg x2 x_y1 x_y1_statq x21 x21_statq stat_q if include_data==1&inlist(stat_q,1,3), clu(subj_id) pformat(%5.4f)



gen safe_1=0
replace safe_1=1 if safe>.5
gen x_y1_safe1=x_y1*safe_1
gen x21_safe1=x21*safe_1
bysort safe_1: reg x2 x_y1 x21 if include_data==1, clu(subj_id)
reg x2 x_y1 x_y1_safe1 x21 x21_safe1 safe_1 if include_data==1, clu(subj_id) pformat(%5.4f)


mat X=J(8,4,.)
forval i=1/4 {
	reg x2 x_y1 x21 if include_data==1&stat_q==`i', clu(subj_id)
	mat X[`i'*2-1,1]=(`i'-1)*5+1
	mat X[`i'*2,1]=(`i'-1)*5+2
	mat ee=e(b)
	mat X[`i'*2-1,2]=ee[1,1]
	mat X[`i'*2,2]=ee[1,2]
	mat eV=e(V)
	mat X[`i'*2-1,3]=eV[1,1]^.5
	mat X[`i'*2,3]=eV[2,2]^.5
}

mat Y=J(4,4,.)
forval i=1/2 {
	reg x2 x_y1 x21 if include_data==1&DG_role==`i'-1, clu(subj_id)
	mat Y[`i'*2-1,1]=(`i'-1)*4+1
	mat Y[`i'*2,1]=(`i'-1)*4+2
	mat ee=e(b)
	mat Y[`i'*2-1,2]=ee[1,1]
	mat Y[`i'*2,2]=ee[1,2]
	mat eV=e(V)
	mat Y[`i'*2-1,3]=eV[1,1]^.5
	mat Y[`i'*2,3]=eV[2,2]^.5
}


mat Z=J(4,4,.)
forval i=1/2 {
	reg x2 x_y1 x21 if include_data==1&safe_1==`i'-1, clu(subj_id)
	mat Z[`i'*2-1,1]=(`i'-1)*5+1
	mat Z[`i'*2,1]=(`i'-1)*5+2
	mat ee=e(b)
	mat Z[`i'*2-1,2]=ee[1,1]
	mat Z[`i'*2,2]=ee[1,2]
	mat eV=e(V)
	mat Z[`i'*2-1,3]=eV[1,1]^.5
	mat Z[`i'*2,3]=eV[2,2]^.5
}
...
gen xmin=X2-1.96*X3
gen xmax=X2+1.96*X3
gen tt=_n-2*floor(_n/2)
twoway (rcap xmax xmin X1 if tt==1, lcol(red)) (rcap xmax xmin X1 if tt==0, lcol(blue)) (scatter X2 X1 if tt==1, col(red) ms(X)) (scatter X2 X1 if tt==0, col(blue) ms(X)), xlab(1.5 "Q1" 6.5 "Q2" 11.5 "Q3" 16.5 "Q4") xtitle("") legend(off) name(gr2) title("by status quartile")
...
gen xmax=Y2+1.96*Y3
gen tt=_n-2*floor(_n/2)
twoway (rcap xmax xmin Y1 if tt==1, lcol(red)) (rcap xmax xmin Y1 if tt==0, lcol(blue)) (scatter Y2 Y1 if tt==1, col(red) ms(X)) (scatter Y2 Y1 if tt==0, col(blue) ms(X)), xlab(1.5 "Recepient" 5.5 "Dictator") xtitle("") name(gr1) title("by DG role") legend(order(1 "Private signal" 2 "Peer action") row(1))
clear
svmat Z
gen xmin=Z2-1.96*Z3
gen xmax=Z2+1.96*Z3
gen tt=_n-2*floor(_n/2)
twoway (rcap xmax xmin Z1 if tt==1, lcol(red)) (rcap xmax xmin Z1 if tt==0, lcol(blue)) (scatter Z2 Z1 if tt==1, col(red) ms(X)) (scatter Z2 Z1 if tt==0, col(blue) ms(X)), xlab(1.5 "5 or fewer" 5.5 "over 5") xtitle("") legend(off) name(gr3) title("by safe choices")
 
grc1leg gr1 gr2 gr3, ycommon row(1) title("Importance of private signals and observed actions") legendfrom(gr1) ysize(6in)
graph export "`path'graph_import.eps", as(eps) preview(off) replace
